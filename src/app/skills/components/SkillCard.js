    ...
    <div>
      ...
      <div
        ...
        className={`p-[0.2rem] rounded-md ${
          isPopupVisible ? 'border-2 border-orange-500' : 'border-2 border-transparent'
        }`}
      >
        <div className="relative w-[13.75rem] h-[12.95rem] rounded-md overflow-hidden cursor-pointer">
          ...
          <div className="relative flex items-center p-[0.2rem] mt-2">
            <div className="w-[3rem] h-[3rem] bg-gray-300 rounded-full flex items-center justify-center ml-4">
              {/* Profile Picture */}
              <img
                src={profilePicture}
                alt={`${name}'s profile`}
      ...
      {isPopupVisible && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <PopUpSkills closePopUp={closePopup} />
        </div>
      )}
    </div>
  );
