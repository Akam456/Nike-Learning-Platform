...
const NoTickerCard = ({ title, backgroundImage, rating, onClick }) => {
  return (
    <div className="w-[16rem] px-2 transform transition-transform duration-300 hover:scale-105 hover:shadow-lg">
    <div onClick={onClick}>
      {/* Card Content */}
      <div
        className="py-4 px-2 relative w-[13.1rem] h-[7.98rem] rounded overflow-hidden bg-cover bg-center"
      ...
      >
        ...
        <div className="absolute top-2 right-2 flex items-center">
          ...
          <span className="ml-1 text-sm text-white font-bold">{rating}</span>
        </div>

        {/* Title (Bottom-Left) */}
        <div className="absolute bottom-1 left-2 text-left">
          <h2 className="text-[1.17rem] font-sans font-[550] text-white">{title}</h2>
        </div>
      </div>
    </div>
    </div>
